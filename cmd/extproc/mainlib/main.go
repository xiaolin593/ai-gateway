...
func Main(ctx context.Context, args []string, stderr io.Writer) (err error) {
	...
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAI, "/v1/files"), extproc.NewFilesProcessor)
	server.Register(path.Join(flags.rootPrefix, endpointPrefixes.OpenAI, "/v1/batches"), extproc.NewBatchesProcessor)
